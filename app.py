...
def generate_text(prompt, max_length=500):
    try:
        ...
        # Show loading spinner to avoid UI freeze
        with st.spinner("🤖 Generating response... please wait"):
            with torch.no_grad():
                outputs = st.session_state.model.generate(
                    **inputs,
                    max_new_tokens=300,       # ↓ Reduced for faster output
                    do_sample=True,
                    temperature=0.7,
                    top_k=50,
                    top_p=0.95,
...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button("🏙️ City Dashboard", key="nav_dashboard"):
        st.session_state.current_page = "Dashboard"
    ...
    st.markdown("---")
    st.markdown("<div style='text-align: center;'>Developed By BAAA's HUB</div>", unsafe_allow_html=True)

# City Dashboard Page
if st.session_state.current_page == "Dashboard":
    st.markdown("<h1 class='header'>🏙️ Smart City Dashboard</h1>", unsafe_allow_html=True)

    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<p>This dashboard provides a quick glance at your city's key sustainability metrics.</p>", unsafe_allow_html=True)

    # Example Metrics
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric(label="Population", value="1.2M", delta="+1.1%")
    with col2:
        st.metric(label="Avg Energy Usage", value="215 kWh", delta="-4.2%")
    with col3:
        st.metric(label="Water Consumption", value="120 L/day", delta="-2.5%")
    with col4:
        st.metric(label="Air Quality Index", value="42 (Good)", delta="-6.3%")

    # Example Plot (Energy Trend)
    st.markdown("<div class='green-card'>", unsafe_allow_html=True)
    st.markdown("<h3 class='subheader'>Energy Consumption Trend</h3>", unsafe_allow_html=True)

    months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul']
    usage = [200, 210, 190, 220, 230, 225, 215]
    trend_df = pd.DataFrame({'Month': months, 'Energy Usage': usage})

    fig = px.line(trend_df, x="Month", y="Energy Usage", markers=True, title="Monthly Energy Usage",
                  line_shape="linear", color_discrete_sequence=["#2e7d32"])
    fig.update_layout(
        plot_bgcolor="white",
        xaxis_title="Month",
        ...
        try:
            ...
            if not all(col in df.columns for col in required_columns):
                ...
                if len(df) < 3:
                    ...
                    with col2:
                        if percent_change > 5:
                            st.markdown("⚠ *Usage is trending up.* Consider energy-saving measures.")
